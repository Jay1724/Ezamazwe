...
import {
  ...
  getMockLessons,
  isDemoEnrolled,
  saveDemoLessonProgress,
} from '../lib/mockData';

const COMPLETE_THRESHOLD = 0.9;
const AUTOSAVE_INTERVAL_MS = 5000;

type ProgressEntry = { completed: boolean; last_watched_seconds: number };
...
export function LessonPlayerPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (isDemoMode) {
        if (!isDemoEnrolled(courseId)) {
          setNotEnrolled(true);
          setLoading(false);
          return;
        }
        const demoLessons = getMockLessons(courseId);
        setCourse(getMockCourse(courseId) ?? null);
        setLessons(demoLessons);
        setProgressByLesson(
          Object.fromEntries(demoLessons.map((l) => [l.id, getDemoLessonProgress(courseId, l.id)]))
        );
        setDemoPercent(getDemoCourseProgressPercent(courseId));
        setLoading(false);
        return;
      }
      ...
      setProgressByLesson(
        Object.fromEntries(
          (progressData ?? []).map((p) => [p.lesson_id, { completed: p.completed, last_watched_seconds: p.last_watched_seconds }])
        )
      );
      setLoading(false);
    }
  ...
  async function saveProgress(secondsWatched: number, forceComplete = false) {
    ...
    if (isDemoMode) {
      saveDemoLessonProgress(courseId, currentLesson.id, secondsWatched, completed);
      setProgressByLesson((prev) => ({ ...prev, [currentLesson.id]: { completed, last_watched_seconds: Math.floor(secondsWatched) } }));
      setDemoPercent(getDemoCourseProgressPercent(courseId));
      return;
    }
    ...
    const updatedMap = { ...progressByLesson, [currentLesson.id]: { completed: data.completed, last_watched_seconds: data.last_watched_seconds } };
    setProgressByLesson(updatedMap);
  ...
  if (!isDemoMode && !profilesLoading && profiles.length === 0) {
    return (
  ...
  const percentComplete = isDemoMode ? demoPercent : Math.round(enrollment?.progress_percent ?? 0);
